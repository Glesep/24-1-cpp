...
GameObject::GameObject(int startX, int startY, int distance): x(startX), y(startY), distance(distance) {}   // 초기 위치와 이동거리 설정
...
void Human::move() {
    ...
    else if (movingKey = 'f' && y < 9)  { y + distance; }
}
...
void Monster::move() {
    ...
    else if (movingKey = 4 && y < 9)  { y + distance; }
}
