...
void EventScheduler::parse_deadline_string(string deadline_str, Date &d) {
    Date tmp(deadline_str.substr(1));
    d = tmp;
}
...
void EventScheduler::add_deadline_event(string date_string, string content) {
    Date d;
    parse_deadline_string(date_string, d);      // ~ 지우기
    DeadlineEvent *e = new DeadlineEvent(d, content);
    events.push_back(e);
}

void EventScheduler::show_relevant_events_Day(Date theDay) {
    for (auto ev: events) {
        if (ev->relevant_to_Day(theDay)) {
            ev->print(cout);
            cout << endl;
        }
    }
...
void EventScheduler::show_relevant_events_Period(Date begin_compared, Date end_compared) {
    for (auto ev: events) {
        if (ev->relevant_to_Period(begin_compared, end_compared)) {
            ev->print(cout);
            cout << endl;
        }
    }
