...
public:
    ...
    LineSegment(int x1, int y1, int x2, int y2): s(x1, y1), t(x2, y2) {}
    ...
    bool intersect(LineSegment &other) {
        if (is_horizontal()) {  // 수평일 때
            return s.y < other.s.y && t.y > other.s.y &&
                        other.s.x < s.x && other.t.x > s.x;
        }
        else {  // 수직일 떄 
            return s.y > other.s.y && s.y < other.t.y &&
                        s.x < other.s.x && t.x > other.s.x;
        }
    }

    // 이 함수에선 두 선분이 교차하지 않는 경우는 고려하지 않는다.
    MyPoint intersectction_point(LineSegment &other) {
        if (is_horizontal()) {  // 수평일 때 교차점
            ...
            for (int v1 = 0; v1 < vlines.size()-1; v1++) {
                for (int v2 = v1+1; v2 < vlines.size(); v2++) {
                    ...
                    if (hlines[h1].intersect(vlines[v1]) &&
                        ...
                        MyRectangle r(hlines[h1].intersectction_point(vlines[v1]),
                                      hlines[h2].intersectction_point(vlines[v2]));    
